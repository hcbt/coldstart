...
# The image OPTIONS, and the mapping from an option set to a built image.
#
# Kept apart from any module system so the same definition can back both
# `flakeModules.default` (flake-parts, for a consumer that still runs a flake)
# and `devenvModules.default` (a plain file a devenv project imports). The two
# wrappers differ only in where the options hang and what they are called.
{ lib }:
let
...
in
{
  ...
  # `pkgs` is the consumer's, so the image is built from THEIR package set.
  buildImages =
    pkgs: images:
    lib.mapAttrs (
      _: image:
